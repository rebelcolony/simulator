      ...
      race.markets.each do |market|
        ...
        ids.each do |id|
          ...
          os.values.each do |runner_id, val|
            ...
            v = val[:best] || val[2]
            Odd.connection.execute("INSERT INTO odds (runner_id, value, market_type, created_at, race_day_id, race_start_at, won, country) VALUES (#{runner_id},#{v},#{market.market_type_id},'#{os.created_at.to_s(:db)}',#{race_day.id},'#{race.start_at.to_s(:db)}', '#{market.winners.include?(runner_id) ? 't' : 'f'}','#{race.venue.country}');")
          end
        end
      end
    end
