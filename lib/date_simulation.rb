class DateSimulation
  ...
  attr_accessor :total, :winners, :best_price, :return, :hit_rate, :interval, :strike_rate
  ...
  def simulate!
    ...
    results = Simulation.connection.select_all("
      SELECT SUM(total) AS total, SUM(winners) AS winners, SUM(best_price) AS best_price, SUM(return) AS return, SUM(profit) AS profit, ROUND((COUNT(profit >= 0) * 100)::numeric / COUNT(*), 2) AS strike_rate
      FROM simulations
